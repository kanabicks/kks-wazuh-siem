...
if [ "$TOTAL_RAM" -lt 8 ]; then
    print_warning "RAM: ${TOTAL_RAM}GB (8GB+ recommended)"
else
...
if [ "$CPU_CORES" -lt 4 ]; then
    print_warning "CPU: ${CPU_CORES} cores (4+ recommended)"
else
...
if [ "$DISK_SPACE" -lt 50 ]; then
    print_error "Disk space: ${DISK_SPACE}GB (50GB+ required)"
    exit 1
else
    print_status "Disk space: ${DISK_SPACE}GB"
fi
...
if ! command -v docker-compose &> /dev/null; then
    print_error "Docker Compose not found! Install: sudo apt install docker-compose"
    exit 1
fi
print_status "Docker Compose: $(docker-compose --version | awk '{print $4}')"
...
if [ ! -d certs ] || [ ! "$(ls -A certs)" ]; then
    ...
    openssl req -new -x509 -sha256 -key root-ca-key.pem -subj "/C=RU/ST=Moscow/L=Moscow/O=KKS/OU=Security/CN=KKS-Root-CA" -out root-ca.pem -days 730
    
    print_info "Generating Admin certificate..."
    openssl genrsa -out admin-key-temp.pem 2048
    openssl pkcs8 -inform PEM -outform PEM -in admin-key-temp.pem -topk8 -nocrypt -v1 PBE-SHA1-3DES -out admin-key.pem
    openssl req -new -key admin-key.pem -subj "/C=RU/ST=Moscow/L=Moscow/O=KKS/OU=Security/CN=admin" -out admin.csr
    ...
    openssl req -new -key wazuh.indexer-key.pem -subj "/C=RU/ST=Moscow/L=Moscow/O=KKS/OU=Security/CN=wazuh.indexer" -out wazuh.indexer.csr
    echo "subjectAltName=DNS:wazuh.indexer,IP:172.18.0.3" > wazuh.indexer.ext
    openssl x509 -req -in wazuh.indexer.csr -CA root-ca.pem -CAkey root-ca-key.pem -CAcreateserial -sha256 -out wazuh.indexer.pem -days 730 -extfile wazuh.indexer.ext
    ...
    openssl req -new -key wazuh.manager-key.pem -subj "/C=RU/ST=Moscow/L=Moscow/O=KKS/OU=Security/CN=wazuh.manager" -out wazuh.manager.csr
    openssl x509 -req -in wazuh.manager.csr -CA root-ca.pem -CAkey root-ca-key.pem -CAcreateserial -sha256 -out wazuh.manager.pem -days 730
    ...
    openssl req -new -key wazuh.dashboard-key.pem -subj "/C=RU/ST=Moscow/L=Moscow/O=KKS/OU=Security/CN=wazuh.dashboard" -out wazuh.dashboard.csr
    openssl x509 -req -in wazuh.dashboard.csr -CA root-ca.pem -CAkey root-ca-key.pem -CAcreateserial -sha256 -out wazuh.dashboard.pem -days 730
...
if [ "$INDEXER_READY" = true ]; then
    ...
    print_warning "Wazuh Indexer is still initializing (check logs)"
fi
...
if docker-compose ps wazuh.manager | grep -q "Up"; then
    ...
    print_warning "Wazuh Manager may still be starting"
fi
...
NGINX_USER=$(grep NGINX_USER .env | cut -d '=' -f2)
